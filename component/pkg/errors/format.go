...
func format(k int, jsonData []map[string]interface{}, str *bytes.Buffer,
	fifo *formatInfo, sep string, flagDetail, flagTrace, modeJSON bool) ([]map[string]interface{},
	*bytes.Buffer) {
	fmt.Println(flagDetail, flagTrace, modeJSON)
	if modeJSON {
		...
		if flagDetail || flagTrace {
			if fifo.stack != nil {
				...
				fmt.Fprintf(str, "%s%s - #%d %s", sep, fifo.err, k, fifo.message)
			}
		} else {
			fmt.Fprintf(str, fifo.message)
		}
	}
